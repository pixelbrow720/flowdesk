"""Tests for engine.flux (FLUX delta-notional flow accumulator).

FLUX is an ISOLATED flow module: it does not touch the Snapshot contract. These
...
def _priced(strike: float, is_call: bool, side: str, size: float, iv: float, t: float = T) -> FluxTrade:
    ...
    return FluxTrade(strike=strike, is_call=is_call, price=px, size=size, side=side, t_expiry=t)
...
def test_unsolvable_iv_returns_none() -> None:
    ...
    bad = FluxTrade(strike=5000.0, is_call=True, price=F * 2.0, size=10.0, side="B", t_expiry=T)
...
def test_explicit_iv_skips_solve() -> None:
    ...
    tr = FluxTrade(strike=5000.0, is_call=True, price=-1.0, size=10.0, side="B", t_expiry=T, iv=0.21)
    dn = signed_delta_notional(tr, F, M_ES, RATE)
    assert dn is not None
...
def test_state_breakdown_and_cumulative() -> None:
    ...
    series = flux_series(trades, F, M_ES, RATE)
...
def test_empty_series() -> None:
    series = flux_series([], F, M_ES, RATE)
...
# --------------------------------------------------------------------------- #
# Redis snapshot round-trip (FLUX unification — docs/architecture/flux-unification.md §4.4 Tier 1)
# --------------------------------------------------------------------------- #
def test_state_to_dict_from_dict_roundtrip() -> None:
    ...
    trades = [
        ...
        FluxTrade(strike=5000.0, is_call=True, price=-1.0, size=1.0, side="B", t_expiry=T),  # bad px -> skipped
    ]
    for tr in trades:
        state.add(tr, F, RATE)
    ...
    restored = FluxState.from_dict(payload)
