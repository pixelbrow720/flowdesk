    ...
    <section id="pricing" className="relative border-t border-[color:var(--hairline)] py-32 md:py-44">
      <div className="container-grid">
        <div className="mb-16 grid grid-cols-12 gap-6 md:mb-24">
          <div className="col-span-12 md:col-span-6">
            ...
            <h2 className="mt-6 text-balance text-4xl font-medium leading-[1.05] md:text-6xl">
              Two tiers. <span className="text-brick">No seat tax.</span>
            </h2>
          </div>
        ...
        <div className="grid grid-cols-12 gap-6">
          {TIERS.map((t, i) => {
            const accentBorder = t.featured ? "border-brick" : "border-teal/40";
            const accentBg = t.featured ? "bg-brick" : "bg-teal";
            const accentBgHover = t.featured ? "hover:bg-brick-deep" : "hover:bg-teal-deep";
            const accentDot = t.featured ? "bg-brick" : "bg-teal";
            ...
            return (
              <motion.div
                ...
                className={`relative col-span-12 flex flex-col rounded-lg border bg-ink-1 p-8 md:col-span-6 md:p-10 ${accentBorder}`}
              >
                {t.featured ? (
                  <span className={`absolute -top-3 left-8 rounded-full ${accentBg} px-3 py-1 font-mono text-[10px] uppercase tracking-[0.18em] text-bone-0`}>
                    Operator
                  </span>
                ) : (
                  <span className="absolute -top-3 left-8 rounded-full border border-teal/60 bg-ink-0 px-3 py-1 font-mono text-[10px] uppercase tracking-[0.18em] text-teal-glow">
                    Studio
                  </span>
                )}
                ...
                <ul className="mt-8 space-y-3 border-t border-[color:var(--hairline)] pt-6">
                  {t.features.map((f) => (
                    <li key={f} className="flex items-start gap-3 text-sm text-bone-1">
                      <span className={`mt-1.5 h-1 w-1 flex-shrink-0 rounded-full ${accentDot}`} />
                      {f}
                    </li>
                  ))}
                </ul>
                ...
                <a
                  ...
                  className={
                    ...
                    (t.featured
                      ? `${accentBg} text-bone-0 ${accentBgHover}`
                      : "border border-teal/50 text-bone-0 hover:border-teal hover:bg-teal/5")
                  }
                >
                  {t.cta} <span>→</span>
                </a>
