...
export function ArcPanel({
  ...
  useEffect(() => {
    ...
    if (w === 0 || h === 0) return; // bail if not yet laid out

    // Scene, camera, renderer.
    const scene = new THREE.Scene();
    scene.background = new THREE.Color(0x000000);

    // Camera defaults: surface should fill most of the canvas.
    //   - FOV 60° gives a wider perspective
    //   - Camera further back but looking more directly at the surface
    ...
    const mkAxis = (
      ...
      // Label sprite. The canvas is sized to the MEASURED text width (+ padding)
      // so long labels like "K (log-moneyness)" are never clipped; the sprite's
      // world scale is then derived from the canvas aspect ratio so the glyph
      // height stays constant regardless of how wide the label is.
      const FONT_PX = 34;
      const PAD = 10;
      ...
      if (ctx) {
        ...
        // measureText is invalidated by the canvas resize → re-set the font.
        ctx.font = `${FONT_PX}px ui-monospace, monospace`;
        ctx.textBaseline = "middle";
        ctx.fillStyle = "#8E8E88";
        ctx.fillText(label, PAD, c.height / 2);
      }
      ...
      // Constant glyph height (0.16 world units); width follows the aspect ratio
      // so the whole label is rendered undistorted and fully on-screen.
      const labelH = 0.16;
    ...
    mkAxis(
      ...
      0x0fb5a8,
      "K (log-moneyness)",
      new THREE.Vector3(+0.6, yOffset - 0.12, +1.32),
    );
    mkAxis(
      corner.clone(),
      ...
      0xb5002e,
      "σ (IV)",
      new THREE.Vector3(-1.2, yOffset + yScale + 0.12, -1),
    );
    mkAxis(
      corner.clone(),
      new THREE.Vector3(-1, yOffset, +1),
      0xfafaf7,
      "t (minutes)",
      new THREE.Vector3(-1.35, yOffset, +0.4),
    );
    scene.add(axes);
    ...
    return () => {
      ...
      renderer.dispose();
      // dispose() alone does NOT free the WebGL context — without this the
      // browser accumulates contexts on every date/bin switch until it hits
      // "Too many active WebGL contexts" and the canvas goes blank / throws.
  ...
  return (
    <div className={`relative flex min-h-0 min-w-0 flex-col ${className}`}>
      <div
        ref={mountRef}
        className="absolute inset-0 overflow-hidden rounded-[4px] border border-rule/40 bg-black"
      />
    </div>
  );
}
